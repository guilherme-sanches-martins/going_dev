...
export default function ColaboradorAudiovisual() {
  const equipamentos = useEquipamentos()
  const jaIniciado = useRef(false) // 🔒 evita execução no primeiro render
  ...
  // 🔹 Adicionar novo equipamento (com bloqueio anti-reload)
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault()

    // 🔒 Evita que React StrictMode ou reload chamem automaticamente
    if (!jaIniciado.current) {
      jaIniciado.current = true
      return
    }

    if (!novo.nome.trim() || !novo.identificacao.trim()) {
      alert('Informe a identificação e o nome do equipamento.')
      return
    }

    // Verifica se a identificação já existe
    const existe = equipamentos.some(eq => eq.identificacao === novo.identificacao)
    if (existe) {
      alert(`❌ Já existe um equipamento com a identificação ${novo.identificacao}.`)
      return
    }
